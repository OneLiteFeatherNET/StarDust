...
public record GameModeCommand(StardustPlugin stardustPlugin) {
    ...
    public void onCommand(@NotNull CommandSender commandSender, @Argument(value = "gamemode") GameMode gameMode, @Argument(value = "player", suggestions = "players") String targetName) {
        ...
        if (!commandSender.hasPermission("featheressentials.command.gamemode.*")) {
            if (!commandSender.hasPermission("featheressentials.command.gamemode." + gameMode.name().toLowerCase())) {
                commandSender.sendMessage(MiniMessage.miniMessage().deserialize(this.stardustPlugin.getMessage("commands.game-mode.no-permission", this.stardustPlugin.getPrefix(), gameMode.name().toLowerCase())));
                return;
            }
        }
        ...
        if (target.equals(commandSender)) {
            ...
            commandSender.sendMessage(Component.translatable("commands.gamemode.success.other").args(this.stardustPlugin.getVaultHook().getDisplayName(target), gameModeName));
        }
    }
